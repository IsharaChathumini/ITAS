...
exports.singleTraining = async (req, res, next) => {
    try {
        const training = await Training.findById(req.params.id).populate('trainingType', 'trainingTypeName'); // Populate trainingType with trainingTypeName
...
exports.deleteTraining = async (req, res, next) => {
    try {
        const training = await Training.findByIdAndDelete(req.params.training_id);
        res.status(200).json({
            success: true,
...
exports.showTrainings = async (req, res, next) => {
    ...
    try {
        const trainings = await Training.find({...keyword,trainingType: categ,location: locationFilter}).sort({ createdAt: -1 }).populate('TrainingType', 'trainingTypeName').populate('user', 'firstName').skip(pageSize * (page - 1)).limit(pageSize)
        res.status(200).json({
            success: true,
            trainings,
            page,
